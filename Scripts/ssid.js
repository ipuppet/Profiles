...
if (argument.get("override") === true) {
  const defaultVal = argument.get("default")
  if (defaultVal) Config.default = parseStringConfig(defaultVal)

  const ssidConfigVal = argument.get("ssidConfig")
  if (ssidConfigVal) Config.ssidConfig = parseSSIDConfig(ssidConfigVal)

  const notificationVal = argument.get("notification")
  if (notificationVal) Config.notification = notificationVal
} else {
  const storage = new Storage()
  storage.setKeyPrefix("ipuppet.boxjs.ssid.")
  Config.default = storage.get("default", Config.default, true)
  Config.ssidConfig = storage.get("modeList", Config.ssidConfig, true)
  Config.notification = storage.get("notification", Config.notification)
}

const ssid = new SSID()
ssid
  .setNotification(NotificationMode[Config.notification])
